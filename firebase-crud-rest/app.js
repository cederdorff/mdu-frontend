const baseUrl = "https://race-crud-rest-default-rtdb.firebaseio.com";
...
async function createUser(name, mail, image) {
    ...
    const res = await fetch(baseUrl + "/users.json", { method: "POST", body: userAsJson });
    console.log(res);
}
...
async function updateUser(id, name, mail, image) {
    ...
    const res = await fetch(baseUrl + `/users/${id}.json`, { method: "PUT", body: userAsJson });
    console.log(res);
}

// === DELETE (DELETE) === //
async function deleteUser(id) {
    const res = await fetch(baseUrl + `/users/${id}.json`, { method: "DELETE" });
    console.log(res);
}
